...
const fs = require ('fs');          //file system module allow us to work with the file on our computer (require method is use to include fs module)
const {exit} = require ("process");
function readfile (){
    let raw = fs.readFileSync('./add.json', 'utf8');    //Unicode Transformation  Format-8: 
    let punishments = JSON.parse(raw);      //parse: parses a string and returns a javascript object
...
function write(punishments){
    let data = JSON.stringify(punishments);   //stringify: Convert JS value to json string
  ...
  let i = 0;
  while (i == 0) {
    console.log("Enter 1 to add contacts, 2 to edit Contacts,3 to Delete Contacts,4 to exit,5 to count,6 to sort by city, 7 to sort by State");
